  ...
  private static Boolean validateDocument(String filename)  {
    try {
      ...
      return true;
   	  	...
   	  	System.err.println("Format error has occured, please verify your file: " + SAXe);
      return false;
    } catch(Exception e){
...
private static void printNodes() {
      ...
      for(int i = 0; i < itemList.getLength(); i++) {
          ...
          if(node.getNodeType()==Node.ELEMENT_NODE) {
              ...
              String cat = item.getAttribute("category");
              NodeList info = item.getChildNodes();

              for (int j = 0; j < info.getLength(); j++) {
                  Node node1 = info.item(j);

                  if(node1.getNodeType()==Node.ELEMENT_NODE) {
                      Element name = (Element) node1;
                      System.out.println(cat + " " + name.getTagName() + ": " + name.getTextContent());
                  }
              }
          }
          System.out.println();
